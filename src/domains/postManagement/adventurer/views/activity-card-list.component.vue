...
export default {
  ...
  errorCaptured(err, instance, info) {
    console.error('Error capturado en ActivityCard:', err, info);
    return false;
  },
  methods: {
    goToDetail() {
...
<template>
  <Card class="activity-card" @click="goToDetail">
    ...
    <template #title>{{ title || 'Sin título' }}</template>
    <template #subtitle>
      <div class="card-meta">
        <span class="meta-item">
          <i class="pi pi-users"></i> {{ people || 0 }} personas
        </span>
        <span class="meta-item">
          <i class="pi pi-tag"></i>S/. {{ price ? price.toFixed(2) : '0.00' }}
        </span>
